...
def read_and_process_data_streaming(csv_path, specific_uri_list=None):
    ...
    if specific_uri_list:
        if isinstance(specific_uri_list, str):
            ...
            log_info(f"开始分析指定URI: {specific_uri_list}")
        elif isinstance(specific_uri_list, list):
            uri_set = set(specific_uri_list)
            log_info(f"开始分析 {len(specific_uri_list)} 个指定URI: {specific_uri_list}")
        else:
            log_info("URI参数格式错误，将分析所有请求")
            uri_set = None
    else:
        uri_set = None
        log_info("开始分析所有请求")
    ...
    try:
        ...
        for chunk_idx, chunk in enumerate(pd.read_csv(csv_path, chunksize=chunk_size)):
            ...
            if total_records % 10000 == 0:
                log_info(f"已读取 {total_records} 条记录，已处理 {processed_records} 条...")
    ...
    log_info(f"流式处理完成")
    log_info(f"总读取记录数: {total_records}")
    if uri_set:
        log_info(f"URI过滤掉记录数: {uri_filtered_records}")
        log_info(f"匹配URI的记录数: {total_records - uri_filtered_records - error_records}")
    log_info(f"成功处理记录数: {processed_records}")
    log_info(f"错误记录数: {error_records}")
    log_info(f"处理耗时: {elapsed:.2f}秒")
    if processed_records > 0:
        log_info(f"处理速度: {processed_records / elapsed:.0f} 记录/秒")

    # 验证是否所有URI都有数据
    if uri_set and len(analyzer.stats['daily']) > 0:
        log_info("URI匹配验证:")
        for uri in uri_set:
            log_info(f"  - {uri}: 已处理")
    elif uri_set:
        log_info(f"警告: 指定的URI列表中没有找到匹配的数据")
        log_info("请检查URI是否正确，或CSV文件中是否包含request_full_uri列")
