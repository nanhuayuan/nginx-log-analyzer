...
# 尝试导入t-digest，如果没有则使用简化版本
try:
    from tdigest import TDigest
    TDIGEST_AVAILABLE = True
except ImportError:
    TDIGEST_AVAILABLE = False
    log_info("t-digest库未安装，使用简化版本分位数算法")
...
class SimpleTDigest:
    ...
    def update(self, value, weight=1):
        ...
        for i, (mean, w) in enumerate(self.centroids):
            if abs(value - mean) < 1e-6:  # 相近的值合并
                new_weight = w + weight
                new_mean = (mean * w + value * weight) / new_weight
    ...
    def _compress(self):
        ...
        while i < len(self.centroids):
            if i + 1 < len(self.centroids):
                mean1, weight1 = self.centroids[i]
                mean2, weight2 = self.centroids[i + 1]
                
                # 合并相邻质心
                new_weight = weight1 + weight2
                new_mean = (mean1 * weight1 + mean2 * weight2) / new_weight
                new_centroids.append((new_mean, new_weight))
                i += 2
            else:
...
class TDigestServiceAnalyzer:
    """基于t-digest的服务分析器 - 使用概率分布计算近似分位数"""
    ...
    def __init__(self, slow_threshold=DEFAULT_SLOW_THRESHOLD):
        ...
        # t-digest分布存储
        self.tdigest_buffers = defaultdict(lambda: defaultdict(lambda: self._create_tdigest()))
        
    def _create_tdigest(self):
        """创建t-digest对象"""
        if TDIGEST_AVAILABLE:
            return TDigest()
        else:
            return SimpleTDigest()
    ...
    def _update_service_stats(self, service_name, group_data):
        ...
        if 'total_request_duration' in group_data.columns:
            ...
            if len(request_times) > 0:
                ...
                # 更新t-digest
                tdigest = self.tdigest_buffers[f'service_{service_name}']['total_request_duration']
        ...
        for metric in all_metrics:
            if metric in group_data.columns:
                ...
                if len(values) > 0:
                    ...
                    # 更新t-digest
                    tdigest = self.tdigest_buffers[f'service_{service_name}'][metric]
                    for value in values:
                        if TDIGEST_AVAILABLE:
                            tdigest.update(value)
                        else:
    ...
    def _update_app_stats(self, app_name, group_data):
        ...
        if 'total_request_duration' in group_data.columns:
            ...
            if len(request_times) > 0:
                ...
                # 更新t-digest
                tdigest = self.tdigest_buffers[f'app_{app_name}']['total_request_duration']
                for value in request_times:
                    if TDIGEST_AVAILABLE:
                        tdigest.update(value)
                    else:
        ...
        for metric in all_metrics:
            if metric in group_data.columns:
                ...
                if len(values) > 0:
                    ...
                    # 更新t-digest
                    tdigest = self.tdigest_buffers[f'app_{app_name}'][metric]
                    for value in values:
                        if TDIGEST_AVAILABLE:
                            tdigest.update(value)
                        else:
    ...
    def generate_final_results(self):
        """生成最终结果"""
        log_info("开始生成t-digest服务分析结果...", show_memory=True)
    ...
    def _add_metrics_to_result(self, result, entity_key, stats):
        ...
        for metric in all_metrics:
            ...
            # 计算分位数（t-digest近似）
            tdigest = self.tdigest_buffers[entity_key][metric]
            if tdigest.total_weight > 0:
                try:
                    if TDIGEST_AVAILABLE:
                        result[f'中位数{display_name}{unit}'] = round(tdigest.percentile(50), 3)
                        result[f'P90{display_name}{unit}'] = round(tdigest.percentile(90), 3)
                        result[f'P95{display_name}{unit}'] = round(tdigest.percentile(95), 3)
                        result[f'P99{display_name}{unit}'] = round(tdigest.percentile(99), 3)
                    else:
                        result[f'中位数{display_name}{unit}'] = round(tdigest.quantile(0.5), 3)
                        result[f'P90{display_name}{unit}'] = round(tdigest.quantile(0.9), 3)
                        result[f'P95{display_name}{unit}'] = round(tdigest.quantile(0.95), 3)
                        result[f'P99{display_name}{unit}'] = round(tdigest.quantile(0.99), 3)
                    
                    # 数据质量指标
                    result[f'{display_name}样本数'] = int(tdigest.total_weight)
                    result[f'{display_name}计算精度'] = 't-digest近似' if TDIGEST_AVAILABLE else '简化近似'
                except:
                    # 如果t-digest计算失败，使用平均值
                    result[f'中位数{display_name}{unit}'] = round(avg_value, 3)
                    result[f'P90{display_name}{unit}'] = round(avg_value, 3)
                    result[f'P95{display_name}{unit}'] = round(avg_value, 3)
                    result[f'P99{display_name}{unit}'] = round(avg_value, 3)
                    result[f'{display_name}样本数'] = int(tdigest.total_weight)
                    result[f'{display_name}计算精度'] = '降级到平均值'
            else:
    ...
    def _cleanup_memory(self):
        """清理内存"""
        self.tdigest_buffers.clear()
        gc.collect()
        log_info("t-digest内存清理完成", show_memory=True)


def analyze_service_performance_tdigest(csv_path, output_path, success_codes=None, slow_threshold=DEFAULT_SLOW_THRESHOLD):
    """基于t-digest的服务性能分析主函数"""
    log_info(f"开始t-digest服务性能分析: {csv_path}", show_memory=True)
    ...
    # 创建分析器
    analyzer = TDigestServiceAnalyzer(slow_threshold)
    ...
    try:
        ...
        # 创建Excel报告
        create_service_performance_excel_tdigest(service_results, app_results, output_path, analyzer.global_stats)
        
        log_info(f"t-digest服务性能分析完成，报告已生成: {output_path}", show_memory=True)
        return service_results.head(5) if not service_results.empty else pd.DataFrame()
        
    except Exception as e:
        log_info(f"t-digest分析出错: {e}")
        raise


def create_service_performance_excel_tdigest(service_results, app_results, output_path, global_stats):
    """创建t-digest服务性能分析Excel报告"""
    log_info(f"开始创建t-digest Excel报告: {output_path}", show_memory=True)
    ...
    if not service_results.empty:
        header_groups = create_service_header_groups_tdigest()
        add_dataframe_to_excel_with_grouped_headers(wb, service_results, '服务性能分析', header_groups)
    
    # 创建应用分析表
    if not app_results.empty:
        app_header_groups = create_app_header_groups_tdigest()
        add_dataframe_to_excel_with_grouped_headers(wb, app_results, '应用性能分析', app_header_groups)
    
    # 创建整体分析表
    create_overall_analysis_sheet_tdigest(wb, service_results, app_results, global_stats)
    
    wb.save(output_path)
    log_info(f"t-digest Excel报告已保存: {output_path}", show_memory=True)


def create_service_header_groups_tdigest():
    """创建服务表头分组"""
    header_groups = {
        '基本信息': ['服务名称', '应用名称'],
...
def create_app_header_groups_tdigest():
    """创建应用表头分组"""
    header_groups = {
        '基本信息': ['应用名称'],
        '请求统计': ['接口请求总数', '成功请求数', '占总请求比例(%)', '成功率(%)'],
...
def create_overall_analysis_sheet_tdigest(wb, service_results, app_results, global_stats):
    """创建整体分析表"""
    ...
    overall_stats = [
        ...
        ['适用场景', '大数据集，内存受限环境'],
        ['算法优势', '内存效率高，计算速度快'],
        ['t-digest库', '已安装' if TDIGEST_AVAILABLE else '未安装（使用简化版）'],
    ]
...
def analyze_service_performance(csv_path, output_path, success_codes=None, slow_threshold=DEFAULT_SLOW_THRESHOLD):
    """主分析函数 - 兼容原接口"""
    return analyze_service_performance_tdigest(csv_path, output_path, success_codes, slow_threshold)