...
class NginxMonitoringGUI:
    ...
    def init_variables(self):
        ...
        self.start_date_var = tk.StringVar(value=(datetime.datetime.now() - datetime.timedelta(days=7)).strftime("%Y-%m-%d"))
        self.end_date_var = tk.StringVar(value=(datetime.datetime.now() - datetime.timedelta(days=1)).strftime("%Y-%m-%d"))
...
• 网络连接超时时间为30秒"""
    ...
    def create_report_tab(self):
        ...
        # 时间配置
        time_frame = ttk.LabelFrame(content_frame, text="时间范围配置", padding=15)
        time_frame.pack(fill=tk.X, pady=(0, 15))
        
        # 开始日期
        ttk.Label(time_frame, text="开始日期:", font=('微软雅黑', 10)).grid(row=0, column=0, sticky=tk.W, pady=8)
        start_date_entry = ttk.Entry(time_frame, textvariable=self.start_date_var, width=20, font=('微软雅黑', 10))
        start_date_entry.grid(row=0, column=1, sticky=tk.W, pady=8, padx=(10, 0))
        ttk.Label(time_frame, text="(格式: YYYY-MM-DD)", font=('微软雅黑', 9), foreground='#7f8c8d').grid(row=0, column=2, sticky=tk.W, padx=(10, 0))
        
        # 结束日期
        ttk.Label(time_frame, text="结束日期:", font=('微软雅黑', 10)).grid(row=1, column=0, sticky=tk.W, pady=8)
        end_date_entry = ttk.Entry(time_frame, textvariable=self.end_date_var, width=20, font=('微软雅黑', 10))
        end_date_entry.grid(row=1, column=1, sticky=tk.W, pady=8, padx=(10, 0))
        ttk.Label(time_frame, text="(格式: YYYY-MM-DD)", font=('微软雅黑', 9), foreground='#7f8c8d').grid(row=1, column=2, sticky=tk.W, padx=(10, 0))
        
        # 快捷时间选择
        shortcut_frame = ttk.Frame(time_frame)
        shortcut_frame.grid(row=2, column=1, columnspan=2, sticky=tk.W, pady=(10, 0))
        
        ttk.Label(shortcut_frame, text="快捷选择:", font=('微软雅黑', 10)).pack(side=tk.LEFT)
        ttk.Button(shortcut_frame, text="昨天", command=lambda: self.set_date_range(-1, -1)).pack(side=tk.LEFT, padx=(10, 5))
        ttk.Button(shortcut_frame, text="最近7天", command=lambda: self.set_date_range(-7, -1)).pack(side=tk.LEFT, padx=5)
        ttk.Button(shortcut_frame, text="最近30天", command=lambda: self.set_date_range(-30, -1)).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(cluster_frame, text="(Prometheus集群标签值)", font=('微软雅黑', 9), foreground='#7f8c8d').grid(row=0, column=2, sticky=tk.W, padx=(10, 0))
        
        # 阈值配置
        threshold_frame = ttk.LabelFrame(content_frame, text="告警阈值配置", padding=15)
        threshold_frame.pack(fill=tk.X, pady=(0, 15))
        
        ttk.Label(threshold_frame, text="高CPU阈值(%):", font=('微软雅黑', 10)).grid(row=0, column=0, sticky=tk.W, pady=8)
        ...
        ttk.Label(threshold_frame, text="高内存阈值(%):", font=('微软雅黑', 10)).grid(row=0, column=2, sticky=tk.W, pady=8, padx=(30, 0))
...
💡 特色功能:
  ...
  • 智能状态判断: 根据阈值自动判断指标状态"""
    ...
    def set_date_range(self, start_days: int, end_days: int):
        """设置日期范围"""
    ...
    def generate_report(self):
        ...
        try:
            ...
            self.log_message(f"📊 收集时间范围: {start_date} 至 {end_date}", "INFO")
            self.log_message(f"🏢 目标集群: {cluster}", "INFO")
            
            # 收集数据
            period_data = self.collector.collect_period_data(cluster, start_date, end_date)
            ...
            if self.output_text_var.get():
                ...
                text_filename = f"{cluster}_监控报告_{self.start_date_var.get()}_{self.end_date_var.get()}_{timestamp}.txt"
            ...
            if self.output_excel_var.get():
                ...
                excel_filename = f"{cluster}_监控数据_{self.start_date_var.get()}_{self.end_date_var.get()}_{timestamp}.xlsx"
